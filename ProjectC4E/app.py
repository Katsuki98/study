...
def chapters_user(user_id,novel_id):
    ...
    novel = Novel.objects.with_id(novel_id)
    all_chapters = novel['chapters']
    return render_template('user/chapters.html', novel_check=novel_check,user=user,all_chapters=all_chapters, novel=novel,user_id=user_id)
        ...
        if username == "":
            ...
            if password == "":
                ...
                if len(found_user) > 0:
                    ...
                    status = "Sai tên tài khoản hoặc mật khẩu"
                    return render_template('login.html', status=status)
...
def delete(user_id,novel_id):
    ...
    if novel_to_delete is not None:
        ...
        user = User.objects.with_id(user_id)
        all_novel = user['novels'] 
        status = 'Novel not found'
        return render_template('user.html', status=status,user=user,user_id=user_id,all_novel=all_novel)
